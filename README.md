...
지난 몇년간  Convolutional neural network(CNN)의 개화를 컴퓨터 비전 분야에서 목격했었다. 많은 CNN 벤치마크 데이터셋은 대체로 object recognition의 정확도에 전진이 있었다. 예를 들어, 120만개의 이미지가 학습된 ImageNet, CNN은 1000개의 카테고리의 물건들을 구분하는데 직접만든 기능들보다 좋은 퍼포먼스를 보여주었다. 뿐만 아니라 이 데이터 세트로 미리 훈련된 CNN은 의미있는 결과를 얻기위해 다른 데이터셋에 옮겨졌다.
